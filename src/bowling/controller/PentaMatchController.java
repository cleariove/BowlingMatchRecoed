...
public class PentaMatchController implements Initializable {
    ...
    public void beginPentaGame() {
        DAO = new InningRecordDAOImpl();
        playerDAO = new PlayerDAOImpl();
        AllPlayers allPlayers = new AllPlayers();
        allPlayers.addAll(playerDAO.getAllPlayer());
        RoundRecords[][] pentaRecords = new RoundRecords[6][10];
        PentaMatch pentaMatch = new PentaMatch(allPlayers,pentaRecords);
        pentaMatch.matchBegin();
        new Pratical().insertRecord(6,60,pentaRecords);
    ...
    public void updatePlayerScore(int i,AllPlayers allPlayers)
    {
        for(int k = 0;k < i;k++)
        {
            ...
            playerDAO.updatePentaScore(allPlayers.get(k).getName(),allPlayers.get(k).getPentaScore());
        }
    }

    
    public void nextInning(int round){
        roundText.setText(String.valueOf(round));
        list.clear();
        Table.refresh();
        DAO =new InningRecordDAOImpl();
        list.addAll(DAO.getMatchScoreByInning(round,"五人赛"));
        setColumn();
        Table.refresh();
    }
    ...
    public void pentaGameTable() {
        ...
        DAO =new InningRecordDAOImpl();
        list.clear();
        list = FXCollections.observableArrayList(DAO.getMatchScoreByInning(1,"五人赛"));
        setColumn();
    }
