...
public class TripleMatchController implements Initializable{
    ...
    private static int i=0;
        ...
        public void beginTripleGame() {
            DAO = new InningRecordDAOImpl();
            playerDAO = new PlayerDAOImpl();
            AllPlayers allPlayers = new AllPlayers();
            allPlayers.addAll(playerDAO.getAllPlayer());
            RoundRecords[][] tripleRecords = new RoundRecords[6][10];
            TripleMatch tripleMatch = new TripleMatch(allPlayers,tripleRecords);
            tripleMatch.matchBegin();
            new Pratical().insertRecord(6,60,tripleRecords);
        ...
        public void updatePlayerScore(int i,AllPlayers allPlayers)
        {
            for(int k = 0;k < i;k++)
            {
                ...
                playerDAO.updateTripleScore(allPlayers.get(k).getName(),allPlayers.get(k).getTripleScore());
            }
        }
        ...
        public void nextInning(int inning){
            ...
            Table.refresh();
            DAO =new InningRecordDAOImpl();
            list.addAll(DAO.getMatchScoreByInning(inning,"三人赛"));
            setColumn();
            Table.refresh();
        }
        ...
        public void tripleGameTable() {
            ...
            DAO =new InningRecordDAOImpl();
            list = FXCollections.observableArrayList(DAO.getMatchScoreByInning(1,"三人赛"));
            setColumn();
        }
